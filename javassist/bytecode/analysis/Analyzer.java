...
public class Analyzer implements Opcode {
   ...
   private Analyzer$ExceptionInfo[] buildExceptionInfo(MethodInfo method) {
      ...
      for(int i = 0; i < table.size(); ++i) {
         ...
         exceptions[i] = new Analyzer$ExceptionInfo(table.startPc(i), table.endPc(i), table.handlerPc(i), type, (Analyzer$1)null);
      }
   ...
   private void mergeExceptionHandlers(IntQueue queue, MethodInfo method, int pos, Frame frame) {
      for(int i = 0; i < this.exceptions.length; ++i) {
         ...
         if (pos >= Analyzer$ExceptionInfo.access$100(exception) && pos < Analyzer$ExceptionInfo.access$200(exception)) {
            Frame newFrame = frame.copy();
            newFrame.clearStack();
            newFrame.push(Analyzer$ExceptionInfo.access$300(exception));
            this.merge(queue, newFrame, Analyzer$ExceptionInfo.access$400(exception));
         }
      }
