...
public abstract class Utils {
      ...
      while(aClass != null) {
         ...
         try {
            if (!descriptor.contains("(")) {
               Field var18 = aClass.isInterface() ? aClass.getField(memberName) : aClass.getDeclaredField(memberName);
               return var18;
            }

            if (isConstructor(descriptor)) {
               Constructor var17 = aClass.isInterface() ? aClass.getConstructor(parameterTypes) : aClass.getDeclaredConstructor(parameterTypes);
               return var17;
            }

            var10000 = aClass.isInterface() ? aClass.getMethod(memberName, parameterTypes) : aClass.getDeclaredMethod(memberName, parameterTypes);
   ...
   public static Field getFieldFromString(String field, ClassLoader... classLoaders) {
      ...
      Field var10000;
      try {
         var10000 = ReflectionUtils.forName(className, classLoaders).getDeclaredField(fieldName);
   ...
   public static Logger findLogger(Class aClass) {
      ...
      try {
         ...
         var10000 = LoggerFactory.getLogger(aClass);
